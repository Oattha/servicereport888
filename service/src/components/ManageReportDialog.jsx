...
import { PRINT_PREVIEW_STORAGE_KEY } from "../pages/PrintPreviewPage";
...
export default function ManageReportDialog({
  ...
  const onPrintClick = async () => {
    ...
    try {
      ...
      if (isMobile) {
        sessionStorage.setItem(PRINT_PREVIEW_STORAGE_KEY, printableHtml);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          response.ok
            ? "Backend ส่งข้อมูลตอบกลับไม่ถูกต้อง"

...
async function prepareStateForStorage(state: ReportRenderState): Promise<ReportRenderState> {
  return {
    ...
    maintenancePlanPage18Values: structuredClone(state.maintenancePlanPage18Values),
    maintenancePlanPage19Values: structuredClone(state.maintenancePlanPage19Values),
    maintenancePlanPage19Signature: { ...state.maintenancePlanPage19Signature },
    signMaintenanceForm: structuredClone(state.signMaintenanceForm),
    signInspectionPage15Choices: { ...state.signInspectionPage15Choices },
    signInspectionPage13State: structuredClone(state.signInspectionPage13State),
    signInspectionPage12State: structuredClone(state.signInspectionPage12State),
    signInspectionPage9State: structuredClone(state.signInspectionPage9State),
    signInspectionPage7State: structuredClone(state.signInspectionPage7State),
    signInspectionPage8State: structuredClone(state.signInspectionPage8State),
    signInspectionPage4State: {
      ...structuredClone(state.signInspectionPage4State),
      mapLocation: {
        ...state.signInspectionPage4State.mapLocation,
        mapScreenshotUrl: await persistUrl(state.signInspectionPage4State.mapLocation.mapScreenshotUrl),
        uploadedImageUrl: await persistUrl(state.signInspectionPage4State.mapLocation.uploadedImageUrl)
      }
    },
    mixingWorkshopPage14Checks: { ...state.mixingWorkshopPage14Checks },
    mixingWorkshopPage15Checks: { ...state.mixingWorkshopPage15Checks },
    mixingWorkshopRemarks: { ...state.mixingWorkshopRemarks },
    mixingWorkshopPage23Checks: { ...state.mixingWorkshopPage23Checks },
    mixingWorkshopPage23Remarks: { ...state.mixingWorkshopPage23Remarks },
    mixingWorkshopPage24Checks: { ...state.mixingWorkshopPage24Checks },
    mixingWorkshopPage24Remarks: { ...state.mixingWorkshopPage24Remarks },
    mixingWorkshopPage25Checks: { ...state.mixingWorkshopPage25Checks },
    mixingWorkshopPage25Remarks: { ...state.mixingWorkshopPage25Remarks },
    mixingWorkshopPage26Checks: { ...state.mixingWorkshopPage26Checks },
    mixingWorkshopPage26Remarks: { ...state.mixingWorkshopPage26Remarks },
    mixingWorkshopPages27To32Checks: structuredClone(state.mixingWorkshopPages27To32Checks),
    mixingWorkshopPages27To32Remarks: structuredClone(state.mixingWorkshopPages27To32Remarks),
    mixingWorkshopPages34To35Choices: structuredClone(state.mixingWorkshopPages34To35Choices),
    mixingWorkshopPages34To35Remarks: structuredClone(state.mixingWorkshopPages34To35Remarks),
...
export async function saveReportDraft(state: ReportRenderState, existingDraft?: ReportDraft | null) {
  ...
  const draft: ReportDraft = {
    ...
    title: `${buildingName} - ${state.templateId.includes("maintenance-plan") ? "แผนบำรุงรักษา" : "รายงานตรวจสอบอาคาร"}`,
    buildingName,
    ownerCompany,
